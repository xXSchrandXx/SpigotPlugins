      ...
      if (args.length != 1) {
        if (API.isUUID(args[1])) {
          ...
          if (portal != null) {
            ...
            NetherPortalGate.getCommandSenderHandler().sendMessageWithoutPrefix(sender, API.getMessage().getString("command.info.message")
            ...
            NetherPortalGate.getCommandSenderHandler().sendMessage(sender, API.getMessage().getString("command.info.noportal").replace("%uuid%", uuid.toString()));
            return true;
          }
        }
        else {
          NetherPortalGate.getCommandSenderHandler().sendMessage(sender, API.getMessage().getString("command.info.nouuid"));
          return true;
        }
      }
      else {
        NetherPortalGate.getCommandSenderHandler().sendMessage(sender, API.getMessage().getString("command.info.usage"));
        return true;
      }
    }
    else {
      NetherPortalGate.getCommandSenderHandler().sendMessage(sender, API.getMessage().getString("nopermission").replace("%permission%", API.getConfig().getString("permissions.command.info")));
      return true;
    }
  }
