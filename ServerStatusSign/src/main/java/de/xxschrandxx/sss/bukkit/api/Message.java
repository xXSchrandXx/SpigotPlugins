 ...
 */
  ...
  public static void sendPlayerStrich(Player p) {
    p.sendMessage(new ComponentBuilder(Loop(API.message.get().getString("strich"))).create());
  }
  ...
  public static void sendStrich(CommandSender sender) {
    if (sender instanceof Player) {
      ...
      API.Log(false, Level.INFO, Loop(API.message.get().getString("strich")));
    }
    else {
      sender.sendMessage(new ComponentBuilder(Loop(API.message.get().getString("strich"))).create());
    }
  }
  ...
  public static void sendMessage(CommandSender sender, String message, net.md_5.bungee.api.chat.HoverEvent.Action hoveraction, String hovervalue, net.md_5.bungee.api.chat.ClickEvent.Action clickaction, String clickvalue) {
    if (sender instanceof Player) {
      ...
      if ((sender != null) && (message != null)) {
        if (!message.isEmpty()) {
          TextComponent textcomponent = new TextComponent(Loop(API.message.get().getString("prefix") + " " + message));
