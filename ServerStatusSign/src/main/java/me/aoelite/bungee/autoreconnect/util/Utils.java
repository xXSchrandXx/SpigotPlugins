...
public class Utils {
    ...
    public static void scheduleAsync(final AutoReconnect instance, final Runnable runnable, long time, TimeUnit timeUnit) {
        ProxyServer.getInstance().getScheduler().schedule(instance.getInstance(), () -> ProxyServer.getInstance().getScheduler().runAsync(instance.getInstance(), runnable), time, timeUnit);
    }
}
